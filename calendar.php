    ...
    <div id="container">
        ...
        <div id="calendar">
            ...
            <?php for ($counter = 1; $counter <= 52; $counter++) {
                ...
                //Show eventAdd inside the week if the user has the correct access level
                echo '<div class="days" ';
                if ($user_level <= 2) {
                    echo 'data-bs-target="#eventAdd" ';
                }
                echo 'data-bs-toggle="modal">' . $counter . '';


                //loop events inside the each week to show multiple events
                $query_events = "SELECT * FROM event WHERE event_starts = $counter OR event_ends = $counter";
                $result = mysqli_query($link, $query_events);
                while ($row = mysqli_fetch_array($result)) {
                    $idevent = $row['idevent'];
                    $title = getCalendarData('event_starts', $counter, 'event_title', $idevent);
                    if ($title == null) {
                        $title = getCalendarData('event_ends', $counter, 'event_title', $idevent);
                    }
                    $event_color = getCalendarData('event_starts', $counter, 'event_colour', $idevent);
                    if ($event_color == null) {
                        $event_color = getCalendarData('event_ends', $counter, 'event_colour', $idevent);
                    }
                    echo '<div class="event" data-bs-target="#eventShow' . $idevent . '" data-bs-toggle="modal" style="background-color: ' . $event_color . ' ;border-radius: 5px">' . $title . '</div>';
                    $description = getCalendarData('event_starts', $counter, 'event_description', $idevent);
                    if ($description == null) {
                        $description = getCalendarData('event_ends', $counter, 'event_description', $idevent);
                    }


                    echo '<div class="modal fade" id="eventShow' . $idevent . '" tabindex="-1" aria-labelledby="eventShowLabel" aria-hidden="true">
                    ...
                    <button type="button" class="btn btn-primary" name="delete-event">Delete Event ' . $idevent . '</button>
